...
class Solution2 {
   public:
    void combine(vector<int>& candidates, int target, vector<vector<int>>& out,
                 vector<int>& curr, int i, int n) {
        if (i == n) {
            if (target == 0) out.push_back(curr);
            return;
        }
        ...
        for (int j = candidates[i]; j <= target; j += candidates[i]) curr.pop_back();
