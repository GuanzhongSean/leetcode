...
class Solution {
 ...
 private:
  void dfs(int n, int i, vector<bool> &&cols, vector<bool> &&diag1, vector<bool> &&diag2,
           int &ans) {
    if (i == n) {
      ++ans;
      return;
...
class Solution2 {
 public:
  void solve(int col, int n, vector<string> &board, int &answer, vector<int> &leftRow,
...
int main() {
  cout << Solution2().totalNQueens(4) << endl;
}
